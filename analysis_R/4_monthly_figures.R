...
# loading data
df <- read.csv("combined_data_monthly.csv")
head(df)
...
### Figure 4 and S4
df0 %>%
	filter(ET_no_gap_count >= 5) %>%
	ggplot(aes(ET_corr,RADET)) +
	geom_point(alpha = 0.2) + geom_abline(lty = 2) +
	geom_smooth(method = "lm",color = "black",formula = 'y~x+0') +
	theme_bw() +
	facet_wrap(~class0,nrow = 2) +
	labs(x = expression("EBR-corrected in situ ET ("*mm~mon^-1*")"),
			 y = expression("RADET ("*mm~mon^-1*")"),
	) +
	stat_poly_eq(
		formula = y ~ x + 0,label.y = 0.95,
		aes(label = paste(
			after_stat(eq.label))),
		parse = TRUE,
		coef.digits = 3,
	...
	stat_regline_equation(aes(label =  paste(..rr.label..)),
												label.y = 322,label.x = 120) +
	geom_point(alpha = 0.2, color = "red",data = df0 %>%filter(!is.na(Ensemble))) +
	geom_smooth(method = "lm",color="red",
							formula = 'y~x+0',
							data = df0 %>%filter(!is.na(Ensemble))) +
	stat_poly_eq(
		formula = y ~ x + 0,label.y = 0.85,
		aes(label = paste(
			after_stat(eq.label))),
		parse = TRUE,
		coef.digits = 3,
		coef.keep.zeros = TRUE,
		data = df0 %>%filter(!is.na(Ensemble)), color = "red"
	) +
	stat_regline_equation(aes(label =  paste(..rr.label..)),
												label.y = 287,label.x = 120,
												data = df0 %>%filter(!is.na(Ensemble)), color = "red")
...
df0 %>%
	...
	ggplot(aes(ET,RADET)) +
	geom_point(alpha = 0.2) + geom_abline(lty=2) + geom_smooth(method = "lm",formula = 'y~x+0') +
	theme_bw() +
	facet_wrap(~class0,nrow = 2) +
	labs(x = expression("EBR-corrected in situ ET ("*mm~mon^-1*")"),
			 y = expression("RADET ("*mm~mon^-1*")"),
	...
	stat_regline_equation(aes(label =  paste(..rr.label..)),
												label.y = 359,label.x = 120)



### Figure 10 (replace ET_corr to ET for Figure S5)
stat_table <- df0 %>%
	ungroup() %>%
...
write.csv(KEG_improvement,"KEG_improvement.csv",row.names = F)
